    ...
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        <button
          ...
          aria-controls="country-combobox"
          aria-expanded={open}
          className="inline-flex h-11 items-center justify-between self-start rounded-bl-lg rounded-tl-lg border border-r-0 shadow-input border-stone-200 bg-white px-4 py-2 text-sm font-medium ring-offset-white transition-colors hover:bg-stone-100 hover:text-slate-900 disabled:pointer-events-none disabled:opacity-50 "
        >
          {value.value ? isoToEmoji(value.value) : "Select country..."}
          <ArrowDown01Icon className="ml-2 size-4 shrink-0 opacity-50" />
        </button>
      </PopoverTrigger>
